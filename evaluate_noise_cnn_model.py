...
with torch.no_grad():
    for images, labels in test_loader:
        ...
        noisy_images = add_gaussian_noise(images, sigma=NOISE_SIGMA)
...
print("\n" + "=" * 60)
print("GAUSSIAN NOISE TEST PERFORMANCE - CNN")
print("=" * 60)
...
plt.title(f"Confusion Matrix - Noisy CNN on MNIST (sigma={NOISE_SIGMA})")
plt.tight_layout()
plt.savefig(f"{VIS_DIR}/CM_noisy_cnn_mnist_sigma{NOISE_SIGMA}.png", dpi=300, bbox_inches="tight")
plt.show()
...
plot_reliability_diagram(
    ...
    all_labels,
    n_bins=10,
    save_path=f"{VIS_DIR}/Reliability_Diagram_noisy_cnn_mnist_sigma{NOISE_SIGMA}.png",
    title=f"Reliability Diagram (Noisy CNN, sigma={NOISE_SIGMA})",
)
...
plt.title(f"Confidence Distribution on Noisy Test Set - CNN (sigma={NOISE_SIGMA})")
plt.legend()
plt.tight_layout()
plt.savefig(f"{VIS_DIR}/Conf_Dist_noisy_cnn_mnist_sigma{NOISE_SIGMA}.png", dpi=300, bbox_inches="tight")
plt.show()